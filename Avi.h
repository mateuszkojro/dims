...
private:
    ...
    struct {
        ...
        size_t number_of_streams_;            /// @brief how many different streams in file (for audio and video - 2)
        size_t size_to_contain_largest_chunk_;/// @brief the biggest size we need to allocate to accommodate any chunk
...
class RawData : public Element {
public:
    void add_child(Element *element) override;
    void parse(std::fstream &fstream) override;
    void set_size(size_t size) override;
    size_t get_size() override;
    Fourcc get_id() override;
    void set_id(const Fourcc &fourcc) override;
...
/**
 * @brief we are recursively building up our Avi structure no interpreting at this stage
